...
**Scoped, not just gated.** A permission decides which screens open; it does not by itself decide what is in them. A teacher sees their own classes, a form teacher the register they own, a house parent their own house and its leave-out. Where the narrower answer is the right one it is applied in the query and enforced again in the action, so a link kept from a colleague does not become a way round it. An admissions interview note, which is prose about a family written in confidence, is read by whoever interviews and whoever decides; everybody else who may open the board is told a note exists and shown the recommendation.

**Termly or monthly billing.** Most schools here bill once a term and that is the default. A school that charges a monthly fee sets the cycle on the fee structure, and can run both at once: monthly tuition alongside a termly examination levy. Only months that have started are offered, because a bill raised in advance is one the school then has to withdraw. A month that straddles two terms is filed under whichever holds more of it, so it is billed once. And a pupil cannot be billed twice for a month: the database refuses it, not merely the screen, because pressing generate twice is the likeliest mistake in monthly billing and the one a parent notices.

**The report to the board.** One document, on the letterhead, assembled from every module rather than by hand out of six screens the week before the meeting. Every figure states what it counted, so it can be checked against the screen it came from. A figure the school cannot state honestly says so rather than appearing as nought, and the front page counts the gaps. Comparisons with the previous term are drawn only where the two periods are of comparable length, rates move in percentage points rather than per cent, and what is set against an annual budget is an annual figure. These sound like small things until a governing board has acted on the version that got them wrong.

**Appraisal, and what it deliberately will not do.** Every part of the system already holds evidence about how a teacher year has gone: notes handed in, registers marked, cover taken on, days away. The appraisal screen puts it in one place, and then stops. Nothing is added up, weighted or turned into a score, because a teacher whose notes are late every week while covering for two absent colleagues is not a worse teacher than one with an empty timetable and immaculate paperwork. The ratings come from somebody who has been in the room; the figures are there so that person is not working from memory. Nobody appraises themselves, only the person being appraised can agree to it, and disagreeing needs words that stay on the file.

**Requisitions, and what the school is committed to.** Departments ask before anything is bought: what is wanted, how many, an estimated price, and which budget it draws on. Whoever approves sees the budget line as it stands and what approving would do to it. Nobody decides their own request, enforced by the database and not only by the screen, and approving something over budget is allowed but asks for a reason that is kept with the name and the date. The point of it is a figure most school systems cannot produce: not what has been spent, but what has been promised and has not yet arrived. A budget line that counts only invoices reads as healthy while four approved requests sit in a drawer, and next year is planned from it.

**Cover.** Leave and the timetable, finally speaking to each other. Approve a teacher for three days and the periods those three days leave show up on one screen, with the best free colleague already chosen: somebody who teaches the subject if there is one, then the department, then whoever has been asked least that day. A period can also be honestly recorded as supervised, merged, or lost with a reason. Nobody is typed in as absent anywhere; the board reads approved leave, so leave cut short on a Tuesday afternoon clears Wednesday by itself.
